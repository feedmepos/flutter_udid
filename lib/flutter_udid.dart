...
import 'package:flutter_udid/src/windows_udid.dart';
...
class FlutterUdid {
  ...
  static Future<String> get udid async {
    if (defaultTargetPlatform == TargetPlatform.windows) {
      return WindowsUDID.uniqueIdentifier().then((value) => value.trim());
    }
  ...
  static Future<String> get consistentUdid async {
    String udid = '';
    if (defaultTargetPlatform == TargetPlatform.windows) {
      udid = await WindowsUDID.uniqueIdentifier().then((value) => value.trim());
    } else {
      udid = await _channel.invokeMethod('getUDID');
    }
